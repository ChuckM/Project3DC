/*
 * Generate a P3DC model file of a cylinder.
 */
#include <stdio.h>
#include <math.h>
...
main(int argc, char *argv[]) {
	...
	printf("#\n# Cylinder Model - autogenerated w/%d segments\n", sides);
	...
	printf("# top_texture -- covers the +Z end of the cylinder.\n");
	printf("# bottom_texture -- covers the -Z end of the cylinder.\n");
	printf("#\n");
	printf("model cylinder {\n");
	printf("	origin 0, 0, 0;\n");
	...
	printf("VB%d <%g, %g>;\n ", sides, 
						((fx(sides)+0.5) * 255.5), ((fy(sides)+0.5) * 255.5));

	/* 
	 * Generate the top
	 */
	printf("	Face <top_texture> (base) ");
	for (i = sides; i > 1; i--) {
		printf("VT%d <%g, %g>, ", i, 
				((fx(i)+0.5) * 255.5), ((fy(i)+0.5) * 255.5));
	}
	...
	y = fy(sides-1);
	printf("	Face <base_texture> (base) ");
	printf("VB%d [%g, %g, 0] <%g, 0.5>, VT%d [%g, %g, 0] <%g, 255.5>, ", 
				sides, x, y, tx(sides-1), sides, x, y, tx(sides-1));
	x = fx(0);
	y = fy(0);
	printf("VT1 [%g, %g, 0] <255.5, 255.5>, VB1 [%g, %g, 0] <255.5, 0.5>;\n ", 
				x, y, x, y);
	printf("}\n");
}
